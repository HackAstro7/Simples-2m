(function() {
    ...
    function atualizarDisplayBanca() {
        ...
        document.getElementById('resultadoTotal').style.color = lucro >= 0 ? '#00ff96' : '#00cc44';
    ...
    function renderHistoricoSinais() {
        ...
        for (let i = signalsHistory.length - 1; i >= 0; i--) {
            ...
            let corClass = s.cor === 'red' ? 'verde' : 'preto';
    ...
    function mostrarNotificacao(cor, forca, padrao) {
        ...
        const corNome = cor === 'red' ? 'VERDE' : 'PRETO';
    ...
    function mostrarNotificacaoWin(cor, martingale) {
        let corTexto = cor === 'red' ? 'VERDE' : 'PRETO';
    ...
    function mostrarNotificacaoLoss(cor) {
        ...
        notif.innerHTML = `<div class="notificacao-header"><span class="notificacao-titulo">❌ LOSS</span><button class="notificacao-fechar">&times;</button></div><div class="notificacao-conteudo"><div><div class="notificacao-cor ${cor}">${corIcone} ${corTexto}</div><div>💀 GALE 2 PERDIDO</div><div style="color:#00cc44;">❌ LOSS!</div></div></div><div class="notificacao-timer"><div class="timer-bar"></div></div>`;
        document.body.appendChild(notif);
        setTimeout(() => notif.classList.add('mostrar'), 10);
        notif.querySelector('.notificacao-fechar').onclick = () => notif.remove();
        setTimeout(() => notif.remove(), 5000);
    ...
    function mostrarNotificacaoProtecao(corSinal) {
        let corTexto = corSinal === 'red' ? 'VERDE' : 'PRETO';
                    ...
                    if (ultimo.cor === 'white') {
                        ...
                        if (currentSignal.martingale < 2) {
                            ...
                            if (currentSignal.martingale === 1) {
                                falar(`Atenção! Sinal perdeu. Entrando no Gale 1 para ${currentSignal.cor === 'red' ? 'verde' : 'preto'}`);
                            } else {
                                falar(`Última chance! Gale 2 ativado para ${currentSignal.cor === 'red' ? 'verde' : 'preto'}`);
                            }
                        } else {
